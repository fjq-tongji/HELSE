from vis_corrector_recap_2 import Corrector
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/llava-7b/Ablation2-m125/Ours_llava_7b_ram_0.5_nouns_2_3.json')
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, processor_blip, model_blip, pipeline, pipeline, sample) #processor_blip, model_blip
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/llava-7b/Ablation2-m125/Ours_llava_7b_groundingdino_captions_5_3.json', 'w', encoding='utf-8') as f:

...
final = []
input_json = './Ours_llava_7b_groundingdino_captions_5_3.json'     ###模块5输出的结果
input_json_2 = './Ours_llava_7b_entity_captions_update_6_3.json'   ###模块6输出的结果
output_json = './Ours_llava_7b_refined_desc_7_3_m12356.json'
...
for k in range(len(stage_6)):
    ...
    ######################## 对阶段6的输出整合成一段话
    refined_passages_stage_6 = ' '.join(stage_7[k]['named_entity_split_sents_update'])   ###阶段6得到的一段话str

    ######################## 对阶段5的输出整合成一段话：a cement mixer is driving on the bridge, a cement mixer is driving on the highway.
    zhenghe_stage5 = []
    for each_sent in stage_6[k]['ram_plus_nouns_captions']:

from vis_corrector_recap_v6_1 import Corrector
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/mini-internvl/Ours_mini_internvl_groundingdino_words_update_4_2_new.json')
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, processor_blip, model_blip, pipeline, pipeline,
                                   pipeline, pipeline, sample) #processor_blip, model_blip
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/mini-internvl/Ours_mini_internvl_groundingdino_captions_5_2_new.json', 'w', encoding='utf-8') as f:

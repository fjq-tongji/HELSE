from vis_corrector_recap_v7 import Corrector
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/mini-internvl/Ours_mini_internvl_groundingdino_captions_5_3.json')
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, pipeline, pipeline, pipeline, pipeline, pipeline, pipeline, sample) #processor_blip, model_blip
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/mini-internvl/Ours_mini_internvl_entity_captions_update_6_3.json', 'w', encoding='utf-8') as f:

...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/llava-13b/Ours_llava_13b_detail_caption_entity_1.json')
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, processor_blip, model_blip, sample) #['output']
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/Ours_llava_7b_entity_blip2_3_1.json', 'w', encoding='utf-8') as f:

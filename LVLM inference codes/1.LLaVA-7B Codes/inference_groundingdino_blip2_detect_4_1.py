...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/llava-7b/RAM-0.3/Ours_llava_7b_ram_0.3_groundingdino_integrate_4_1.json')
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, processor_blip,model_blip, pipeline,pipeline, sample) #['output']
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/llava-7b/RAM-0.3/Ours_llava_7b_ram_0.3_groundingdino_blip2_detect_4_1.json', 'w', encoding='utf-8') as f:

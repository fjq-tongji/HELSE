...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/Ours_llava_7b_entity_blip2_3_3.json')
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, pipeline, pipeline, processor_instructblip, model_instructblip, sample) #processor_blip, model_blip
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/Ours_llava_7b_entity_instructblip_3_3.json', 'w', encoding='utf-8') as f:

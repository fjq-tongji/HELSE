...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/llava-7b/RAM-0.3/Ours_llava_7b_ram_0.3_groundingdino_blip2_detect_4_3.json')
    ...
    parser.add_argument('--detector-config',
                        default='/home/fjq/MLLMs/Woodpecker/groundingdino/config/GroundingDINO_SwinT_OGC.py', type=str, help="Path to the detector config, \
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, pipeline,pipeline, processor_instructblip,model_instructblip, sample) #['output']
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/llava-7b/RAM-0.3/Ours_llava_7b_ram_0.3_groundingdino_instructblip_detect_4_3.json', 'w', encoding='utf-8') as f:

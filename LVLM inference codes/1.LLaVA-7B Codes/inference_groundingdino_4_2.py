...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage-1', default='./Each_stage_texts/Ours_llava_7b_ram_0.5_nouns_2_2.json')
    ...
    for sample in coda_detail_captions:
        output = corrector.correct(pipeline, pipeline,pipeline,pipeline,pipeline, sample) #['output']
        print(output)
        coda_detail_captions_correct.append(output)
        print(len(coda_detail_captions_correct))


    with open('./Each_stage_texts/Ours_llava_7b_ram_0.5_groundingdino_4_2.json', 'w', encoding='utf-8') as f:

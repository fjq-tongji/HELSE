...
total_dict_val = []
total_dict_test = []
with open('./Ours/CODA_mplug_owl3_adversarial_answers_ours.json', 'r', encoding='utf-8') as f:
    for line in f:
        line = line.strip()
        if line:
            # 解析每一行的JSON数据
...
with open('./Ours/CODA_mplug_owl3_adversarial_answers_ours_val.json', 'w', encoding='utf-8') as f:
    for d in total_dict_val:
        json.dump(d, f, ensure_ascii=False)
        f.write('\n')
with open('./Ours/CODA_mplug_owl3_adversarial_answers_ours_test.json', 'w', encoding='utf-8') as f:

...
pretrained_pth = '/data/fjq/3.VLMS/clip-vit-base-patch32/seem_focall_v1.pt'
...
def segment(img_path_file, seg_num):
    result_list = []
    img_file_path = '/data/fjq/3.VLMS/CODA2022/images_total/'

    img_list = json.loads(open(img_path_file, 'r').read())
    #img_list = random.random(img_list, k=seg_num)
    
    segment_result_file = open(f'./{img_path_file.split(".json")[0].split("/")[-1]}_segmentation_result.json', 'w')
    for data in tqdm.tqdm(img_list):
        image_path = data['image']
        image = {'image': Image.open(img_file_path + image_path).convert('RGB')}
        categories = inference(image = image, task = ['Panoptic'])

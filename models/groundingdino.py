...
class Detector:
    ...
    def detect_objects(self, sample: Dict, pipeline):
        img_path = sample['img_path']
        extracted_entities = sample['ram_plus_results']   ###ram_plus_nouns_integrate
        ...
        for entity_str in extracted_entities:
            ...
            boxes, logits, phrases = predict(
                model=self.model,
                image=image,
                caption=entity_str,     #####要检测的是 man.bus.car.truck，给groundingdino
            ...
            #print('--------')
            #print(global_entity_dict)
            final.append(global_entity_dict)

        # sample['ram_plus_nouns_groundingdino'] = final

        final_ram_nouns = []
        for item in final:
            for key, value in item.items():
                if value['total_count'] > 0:
                    final_ram_nouns.append(key)
        sample['ram_plus_nouns_groundingdino'] = final_ram_nouns
